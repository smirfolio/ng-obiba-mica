 ...
 * @constructor
 */
function AbstractCriteriaBuilder(a,b,c,d,e){/**
...
this.newCriteriaItemBuilder=function(){return new CriteriaItemBuilder(d,e)},this.initialize=function(f){this.target=f,this.rootRql=a,this.taxonomies=c,this.LocalizedValues=d,this.lang=e,this.rootItem=this.newCriteriaItemBuilder().parent(b).type(this.target).rqlQuery(this.rootRql).target(this.target).build()},/**
   ...
   * @param node
   */
this.buildLeafItem=function(a,b,c,d,e){var f=this,g=this.taxonomies.filter(function(b){return a===b.name});if(0===g.length)throw new Error("Could not find taxonomy:",a);g=g[0];var h=g.vocabularies.filter(function(a){return b===a.name});if(0===h.length)throw new Error("Could not find vocabulary:",b);var i=0;h=h[0];var j=new CriteriaItemBuilder(f.LocalizedValues,f.lang).type(d.name).target(f.target).taxonomy(g).vocabulary(h).rqlQuery(d).parent(e);return h.terms&&h.terms.some(function(a){return-1!==c.indexOf(a.name)?(j.selectedTerm(a).build(),i++,i===c.length):void 0}),j.build()}}/* global CRITERIA_ITEM_EVENT */
...
angular.module("obiba.mica.search",["obiba.alert","ui.bootstrap","pascalprecht.translate","templates-ngObibaMica"]).config(["$provide",function(a){a.provider("ngObibaMicaSearchTemplateUrl",(new NgObibaMicaTemplateUrlFactory).create({list:{header:null,footer:null},view:{header:null,footer:null},form:{header:null,footer:null}}))}]);/* exported QUERY_TYPES */
...
LIMIT:"limit",SORT:"sort",AND:"and",OR:"or",NOT:"not",FACET:"facet",LOCALE:"locale",AGGREGATE:"aggregate",BUCKET:"bucket",/* leaf criteria nodes */
IN:"in",OUT:"out",EQ:"eq",GT:"gt",GE:"ge",LT:"lt",LE:"le",BETWEEN:"between",MATCH:"match",EXISTS:"exists",MISSING:"missing"};/**
 * This method is where a criteria gets created
 */
AbstractCriteriaBuilder.prototype.visitLeaf=function(){},/**
 * Returns all the criterias found
 * @returns {Array}
 */
AbstractCriteriaBuilder.prototype.getRootItem=function(){return this.rootItem},/**
 * Node condition visitor
 * @param node
 * @param parentItem
 */
AbstractCriteriaBuilder.prototype.visitCondition=function(a,b){var c=this.newCriteriaItemBuilder().parent(b).rqlQuery(a).type(a.name).build();b.children.push(c),this.visit(a.args[0],c),this.visit(a.args[1],c)},/**
 * Node not visitor
 * @param node
 * @param parentItem
 */
AbstractCriteriaBuilder.prototype.visitNot=function(a,b){var c=this.newCriteriaItemBuilder().parent(b).rqlQuery(a).type(a.name).build();b.children.push(c),this.visit(a.args[0],c)},/**
 * General purpose node visitor
 * @param node
 * @param parentItem
 */
AbstractCriteriaBuilder.prototype.visit=function(a,b){
 ...
 */
AbstractCriteriaBuilder.prototype.build=function(){var a=this;this.rootRql.args.forEach(function(b){a.visit(b,a.rootItem)})};/**
 ...
 * @constructor
 */
var VariableCriteriaBuilder=function(a,b,c,d,e){AbstractCriteriaBuilder.call(this,a,b,c,d,e),this.initialize(RQL_NODE.VARIABLE),this.parserFieldName=function(a){var b=/(\w+)__(\w+)/,c=b.exec(a.replace(/attributes\./,"").replace(/\.\w+$/,""));if(c&&c.length>2)return{taxonomy:c[1],vocabulary:c[2]};throw new Error("Invalid field name",a)}};VariableCriteriaBuilder.prototype=Object.create(AbstractCriteriaBuilder.prototype),VariableCriteriaBuilder.prototype.visitLeaf=function(a,b){console.log("VariableCriteriaBuilder.visitLeaf");var c=a.args[0],d=a.args[1],e=this.parserFieldName(c),f=this.buildLeafItem(e.taxonomy,e.vocabulary,d instanceof Array?d:[d],a,b);b.children.push(f)},angular.module("obiba.mica.search").service("RqlQueryUtils",[function(){/**
     * Finds the parent node to which new queries can be added
     *
     * @param targetNode
     * @returns {*}
     */
function a(a){var b=a.args.filter(function(a){switch(a.name){case RQL_NODE.AND:case RQL_NODE.OR:case RQL_NODE.NOT:case RQL_NODE.IN:case RQL_NODE.OUT:case RQL_NODE.EQ:case RQL_NODE.GT:case RQL_NODE.GE:case RQL_NODE.LT:case RQL_NODE.LE:case RQL_NODE.BETWEEN:case RQL_NODE.MATCH:case RQL_NODE.EXISTS:case RQL_NODE.MISSING:return!0}return!1}).pop();return b?a.args.findIndex(function(a){return a.name===b.name}):-1}this.variableQuery=function(){return new RqlQuery(QUERY_TARGETS.VARIABLE)},this.eqQuery=function(a,b){var c=new RqlQuery(RQL_NODE.EQ);return c.args.push(b),c},this.inQuery=function(a,b){var c=b&&b.length>0,d=c?RQL_NODE.IN:RQL_NODE.EXISTS,e=new RqlQuery(d);return e.args.push(a),c&&e.args.push(b),e},this.updateInQuery=function(a,b,c){var d=b&&b.length>0;return a.name=d?RQL_NODE.IN:c?RQL_NODE.MISSING:RQL_NODE.EXISTS,d?a.args[1]=b:a.args.splice(1,1),a},/**
     * Helper finding the vocabulary field, return name if none was found
     *
     * @param vocabulary
     * @returns {*}
     */
this.vocabularyFieldName=function(a){var b=a.attributes.filter(function(a){return"field"===a.key}).pop();return b?b.value:a.name},/**
     * Creates a RqlQuery from an item
     *
     * @param item
     * @returns {RqlQuery}
     */
this.buildRqlQuery=function(a){
// TODO take care of other type (min, max, in, ...)
return this.inQuery(this.vocabularyFieldName(a.vocabulary),a.vocabulary.terms.map(function(a){return a.name}))},/**
...
this.builders={/**
         ...
         * @returns {*}
         */
variable:function(d,e,f){function g(a,b){var d=new VariableCriteriaBuilder(a,b,i,c,f);d.build(),h.resolve(d.getRootItem())}var h=a.defer();return i?g(d,e):b.get({target:QUERY_TARGETS.VARIABLE}).$promise.then(function(a){i=a,g(d,e)}),h.promise},/**
         * Builds the Vairable criteria tree
         *
         * @param rootRql
         * @param rootItem
         ...
         * @returns {*}
         */
study:function(c){function d(){/*
             var builder = new VariableCriteriaBuilder(node, self.variableTaxonomies, LocalizedValues, lang);
             builder.build();*/
e.resolve({})}var e=a.defer();return j?d(c):b.get({target:QUERY_TARGETS.STUDY}).$promise.then(function(a){j=a,d(c)}),e.promise}},/**
       ...
       * @returns {*}
       */
this.createCriteria=function(b,c){var d=a.defer(),e=(new CriteriaItemBuilder).type(RQL_NODE.AND).rqlQuery(b).build();if(0===b.args.length)return d.resolve(e),d.promise;var f=[],g=this,h=0;return b.args.forEach(function(a){QUERY_TARGETS[a.name.toUpperCase()]&&f.push(a)}),f.forEach(function(a){g.builders[a.name](a,e,c).then(function(a){e.children.push(a),h++,h===f.length&&d.resolve(e)})}),d.promise},/**
       ...
       */
this.prepareCriteriaTermsQuery=function(a,b,c){var e=(new RqlParser).parse(b),f=new RqlQuery("aggregate"),g=new RqlQuery("facet");return f.args.push(d.vocabularyFieldName(c)),e.args.some(function(b){return b.name===a?(b.args.push(f),!0):!1}),e.args.push(g),e.serializeArgs(e.args)},/**
