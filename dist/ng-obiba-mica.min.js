...
function typeToTarget(a){switch(a.toLocaleString()){case QUERY_TYPES.NETWORKS:return QUERY_TARGETS.NETWORK;case QUERY_TYPES.STUDIES:return QUERY_TARGETS.STUDY;case QUERY_TYPES.DATASETS:return QUERY_TARGETS.DATASET;case QUERY_TYPES.VARIABLES:return QUERY_TARGETS.VARIABLE}throw new Error("Invalid type: "+a)}/* exported CriteriaItem */
function CriteriaItem(a){var b=this;Object.keys(a).forEach(function(c){b[c]=a[c]})}/* exported RepeatableCriteriaItem */
 ...
 * @constructor
 */
function CriterionState(){var a=[],b=[];this.dirty=!1,this.open=!1,this.loading=!0,this.addOnOpen=function(b){a.push(b)},this.addOnClose=function(a){b.push(a)},this.onOpen=function(){a.forEach(function(a){a()})},this.onClose=function(){b.forEach(function(a){a()})}}/**
 ...
 */
CriteriaBuilder.prototype.visitLeaf=function(a,b){var c=RQL_NODE.MATCH===a.name,d=a.args[c?1:0],e=a.args[c?0:1],f=this.fieldToVocabulary(d),g=this.buildLeafItem(f.taxonomy,f.vocabulary,e instanceof Array?e:[e],a,b),h=this.leafItemMap[g.id];h?h.isRepeatable()?h.addItem(g):(console.error("Non-repeatable criteria items must be unique,",h.id,"will be overwritten."),h=g):h=g.vocabulary.repeatable?(new RepeatableCriteriaItem).addItem(g):g,this.leafItemMap[g.id]=h,b.children.push(g)},/**
...
this.buildRqlQuery=function(a){return this.isNumericVocabulary(a.vocabulary)?this.rangeQuery(this.criteriaId(a.taxonomy,a.vocabulary),null,null):this.isMatchVocabulary(a.vocabulary)?this.matchQuery(this.criteriaId(a.taxonomy,a.vocabulary),null):this.inQuery(this.criteriaId(a.taxonomy,a.vocabulary),a.term?a.term.name:void 0)},/**
     ...
     * @returns {*}
     */
this.addQuery=function(b,c,d){if(0===b.args.length)b.args.push(c);else{var e=a(b);if(-1===e)b.args.push(c);else{var f=b.args.splice(e,1).pop();
...
if(!d&&c.args&&c.args.length>0){var g="Mica_"+b.name;d=c.args[0].startsWith(g+".")?RQL_NODE.AND:RQL_NODE.OR}var h=new RqlQuery(d||RQL_NODE.AND);h.args.push(f,c),b.args.push(h)}}return b},/**
...
this.createCriteria=function(b,c){var d=a.defer(),f=(new CriteriaItemBuilder).type(RQL_NODE.AND).rqlQuery(b).build(),g={};if(!e.hasTargetQuery(b))return d.resolve({root:f,map:g}),d.promise;var h=[],i=this,j=0;return b.args.forEach(function(a){QUERY_TARGETS[a.name.toUpperCase()]&&h.push(a)}),h.forEach(function(a){i.builders(a.name,a,f,c).then(function(a){f.children.push(a.root),g=angular.extend(g,a.map),j++,j===h.length&&d.resolve({root:f,map:g})})}),d.promise},/**
       ...
       */
this.prepareCriteriaTermsQuery=function(a,b,c){function d(a,b,c){if(!a||!a.args)return null;if((a.name===RQL_NODE.IN||a.name===RQL_NODE.MISSING||a.name===RQL_NODE.CONTAINS)&&a.args[0]===b)return a;for(var e=a.args.length;e--;){var f=d(a.args[e],b,c);f&&(a.args[e]=c)}}var f=(new RqlParser).parse(a),g=f.args.filter(function(a){return a.name===b.target}).pop();if(g){var h=new RqlQuery(RQL_NODE.EXISTS),i=e.criteriaId(b.taxonomy,b.vocabulary);h.args.push(i),d(g,i,h),g.args.push(e.aggregate([i])),g.args.push(e.limit(0,0))}return f.args.push(new RqlQuery(RQL_NODE.FACET)),c&&e.addLocaleQuery(f,c),f.serializeArgs(f.args)},this.prepareSearchQuery=function(a,b,c,d,g){var h=angular.copy(b),i=typeToTarget(a);e.addLocaleQuery(h,d);var j=f(i,h);j||(j=new RqlQuery(i),h.args.push(j));var k=c[i]||{from:0,size:10};return e.addLimit(j,e.limit(k.from,k.size)),g&&e.addSort(j,g),(new RqlQuery).serializeArgs(h.args)},/**
...
// facet
return d.args.forEach(function(a){if("study"===a.name){i=!0;var b=null;h=a.args.filter(function(a,c){return"limit"===a.name&&(b=c),["limit","sort","aggregate"].indexOf(a.name)<0}).length,null!==b&&a.args.splice(b,1),g=a}}),i||(g=new RqlQuery("study"),d.args.push(g)),h||g.args.push(new RqlQuery(RQL_NODE.MATCH)),g.args.push(e),d.args.push(new RqlQuery("facet")),d.serializeArgs(d.args)},this.getTargetAggregations=function(a,b,c){/**
